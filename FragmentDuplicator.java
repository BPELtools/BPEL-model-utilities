...
public class FragmentDuplicator {
	...
	public static Activity copyActivity(Activity origAct, Process fragProc, Definition fragDefn) {
		...
		if (origAct instanceof Empty)
			newActivity = copyEmpty((Empty) origAct);
		else if (origAct instanceof Invoke)
			newActivity = copyInvoke((Invoke) origAct, fragProc, fragDefn);
		else if (origAct instanceof Assign)
			newActivity = copyAssign((Assign) origAct);
		else if (origAct instanceof Reply)
			newActivity = copyReply((Reply) origAct, fragProc, fragDefn);
		else if (origAct instanceof Receive)
			newActivity = copyReceive((Receive) origAct, fragProc, fragDefn);
		else if (origAct instanceof Wait)
			newActivity = copyWait((Wait) origAct);
		else if (origAct instanceof Throw)
			newActivity = copyThrow((Throw) origAct);
		else if (origAct instanceof Exit)
			newActivity = copyExit((Exit) origAct);
		else if (origAct instanceof Flow)
			newActivity = copyFlow((Flow) origAct);
		else if (origAct instanceof If)
			newActivity = copyIf((If) origAct);
		else if (origAct instanceof While)
			newActivity = copyWhile((While) origAct);
		else if (origAct instanceof Sequence)
			newActivity = copySequence((Sequence) origAct);
		else if (origAct instanceof Pick)
			newActivity = copyPick((Pick) origAct);
		else if (origAct instanceof Scope)
			newActivity = copyScope((Scope) origAct);
		else if (origAct instanceof Compensate)
			newActivity = copyCompensate((Compensate) origAct);
		else if (origAct instanceof CompensateScope)
			newActivity = copyCompensateScope((CompensateScope) origAct);
		else if (origAct instanceof Rethrow)
			newActivity = copyRethrow((Rethrow) origAct);
		else if (origAct instanceof OpaqueActivity)
			newActivity = copyOpaqueActivity((OpaqueActivity) origAct);
		else if (origAct instanceof ForEach)
			newActivity = copyForEach((ForEach) origAct);
		else if (origAct instanceof RepeatUntil)
			newActivity = copyRepeatUntil((RepeatUntil) origAct);
		else if (origAct instanceof Validate)
			newActivity = copyValidate((Validate) origAct);
		else if (origAct instanceof ExtensionActivity)
			newActivity = copyExtensionActivity((ExtensionActivity) origAct);
	...
	public static Invoke copyInvoke(Invoke origAct, Process fragProc, Definition fragDefn) {
		...
		copyStandardAttributes(origAct, newInvoke);
		...
		if (newPlink == null) {
			newPlink = copyPartnerLink(origAct.getPartnerLink());
		}
		...
		if (newPt == null) {
			newPt = copyPortType(origAct.getPortType());
		}
		...
		if (newOp == null) {
			newOp = copyOperation(origAct.getOperation());
		}
		...
		if (origAct.getInputVariable() != null) {
			...
			if (inputVar == null)
				throw new IllegalStateException("Variable " + inputVarName
						+ " is not found in fragment process " + fragProc.getName());
		...
		if (origAct.getOutputVariable() != null) {
			...
			if (outputVar == null)
				throw new IllegalStateException("Variable " + outputVarName
						+ " is not found in fragment process " + fragProc.getName());
		...
		if (origAct.getCorrelations() != null) {
			...
			Correlations newCorrelations = copyCorrelations(origAct.getCorrelations(), fragProc);
		...
		copyStandardElements(origAct, newInvoke);
		...
		for (Correlation origCorrel : origCorrelations.getChildren()) {
			...
			if (newCorrelSet == null)
				throw new IllegalStateException(
						"the fragment process does not contain any correlation set, check out the ProcessFragmenter.addCorrelation.");
	...
	public static Assign copyAssign(Assign act) {
		...
		copyStandardAttributes(act, newAssign);

		List<Copy> copyList = act.getCopy();
		for (Copy copy : copyList) {
			newAssign.getCopy().add(copyCopy(copy));
		}

		copyStandardElements(act, newAssign);
		return newAssign;
	}
	...
	public static Reply copyReply(Reply origAct, Process fragProc, Definition fragDefn) {
		...
		copyStandardAttributes(origAct, newReply);
		...
		copyStandardElements(origAct, newReply);
	...
	public static Receive copyReceive(Receive origAct, Process fragProc, Definition fragDefn) {
		...
		copyStandardAttributes(origAct, newReceive);
		...
		copyStandardElements(origAct, newReceive);
	...
	public static void copyStandardAttributes(Activity origAct, Activity newAct) {
		...
		if (origAct == null || newAct == null) {
			throw new NullPointerException("argument is null, origAct==null:" + (origAct == null)
	...
	public static void copyStandardElements(Activity origAct, Activity newAct) {
		...
		if (origAct == null || newAct == null)
			throw new NullPointerException("Null parameter error! origAct==null:"
					+ (origAct == null) + " newAct==null:" + (newAct == null));

		// sources
		if (origAct.getSources() != null) {
			copySources(origAct.getSources(), newAct);
		}

		// targets
		if (origAct.getTargets() != null) {
			copyTargets(origAct.getTargets(), newAct);
		}
	}
	 ...
	 * @param container
	 *            The container of the new sources
	 */
	public static void copySources(Sources origSources, Activity container) {
		if (origSources == null || container == null)
			throw new NullPointerException("argument is null. origSources == null:"
					+ (origSources == null) + " container == null:" + (container == null));
		...
		for (Source origSource : origSources.getChildren()) {
			...
			// transitionCondition.
			newSource.setLink(copyLink(origSource.getLink()));
			if (origSource.getTransitionCondition() != null)
				newSource
						.setTransitionCondition(copyCondition(origSource.getTransitionCondition()));
	 ...
	 * @param container
	 *            The container activity of the new Targets
	...
	public static void copyTargets(Targets origTargets, Activity container) {
		...
		if (origTargets == null || container == null)
			throw new NullPointerException("argument is null. origTargets == null:"
					+ (origTargets == null) + " container == null:" + (container == null));
		...
		if (origTargets.getJoinCondition() != null)
			newTargets.setJoinCondition(copyCondition(origTargets.getJoinCondition()));
		...
		for (Target origTarget : origTargets.getChildren()) {
			...
			if (origTarget.getLink() != null)
				newTarget.setLink(copyLink(origTarget.getLink()));
	...
	public static Copy copyCopy(Copy copy) {
		...
		newCopy.setFrom(copyFrom(copy.getFrom()));
		newCopy.setTo(copyTo(copy.getTo()));
	...
	public static From copyFrom(From from) {
		...
		if (from.getVariable() != null)
			newFrom.setVariable(copyVariable(from.getVariable()));
		if (from.getPart() != null)
			newFrom.setPart(copyPart(from.getPart()));
		if (from.getProperty() != null)
			newFrom.setProperty(copyProperty(from.getProperty()));
		if (from.getQuery() != null)
			newFrom.setQuery(copyQuery(from.getQuery()));
		if (from.getLiteral() != null)
			newFrom.setLiteral(from.getLiteral());
		if (from.getExpression() != null)
			newFrom.setExpression(copyExpression(from.getExpression()));
		if (from.getPartnerLink() != null)
			newFrom.setPartnerLink(copyPartnerLink(from.getPartnerLink()));
		return newFrom;
	}
	...
	public static To copyTo(To to) {
		...
		if (to.getVariable() != null)
			newTo.setVariable(copyVariable(to.getVariable()));
		if (to.getPart() != null)
			newTo.setPart(copyPart(to.getPart()));
		if (to.getProperty() != null)
			newTo.setProperty(copyProperty(to.getProperty()));
		if (to.getQuery() != null)
			newTo.setQuery(copyQuery(to.getQuery()));
		if (to.getExpression() != null)
			newTo.setExpression(copyExpression(to.getExpression()));
		if (to.getPartnerLink() != null)
			newTo.setPartnerLink(copyPartnerLink(to.getPartnerLink()));
		return newTo;
	}
	...
	public static Message copyMessage(Message message) {
		...
		QName newQName = copyQName(message.getQName());
		newMessage.setQName(newQName);

		List<Part> parts = message.getEParts();

		for (Part part : parts) {
			Part newPart = copyPart(part);
	...
	public static Part copyPart(Part origPart) {
		...
		if (origPart.getTypeName() != null) {
			newPart.setTypeName(copyQName(origPart.getTypeName()));
		}

		if (origPart.getElementDeclaration() != null) {
			logger.warn("The message part \"" + origPart.getName()
					+ "\" type is element declaration!");
		}
	...
	public static PortType copyPortType(PortType portType) {
		...
		while (iterator.hasNext()) {
			...
			Operation newOperation = copyOperation(origOperation);
	...
	public static Operation copyOperation(Operation origOperation) {
		...
		if (origOperation.getInput() != null)
			newOperation.setInput(copyInput(origOperation.getEInput()));
		if (origOperation.getOutput() != null)
			newOperation.setOutput(copyOutput(origOperation.getEOutput()));
	...
	public static Operation copyOperation(Operation origOperation, Definition fragDefn) {
		...
		if (origInput != null) {
			...
			if (newMessage == null) {
				throw new IllegalStateException("Can not find message in fragment definition: "
						+ origMessage.getQName());
			}
	...
	public static Input copyInput(Input input) {
		...
		if (input.getEMessage() != null)
			newInput.setMessage(copyMessage(input.getEMessage()));
		return newInput;
	}
	...
	public static Output copyOutput(Output output) {
		...
		if (output.getMessage() != null)
			newOutput.setMessage(copyMessage(output.getEMessage()));
	...
	public static PartnerLink copyPartnerLink(PartnerLink origPartnerLink) {
		...
		PartnerLinkType newPLT = copyPartnerLinkType(oldPLT);
	...
	public static PartnerLinkType copyPartnerLinkType(PartnerLinkType origPartnerLinkType) {
		...
		for (Role origRole : origPartnerLinkType.getRole()) {
			Role newRole = copyRole(origRole);
	...
	public static Role copyRole(Role origRole) {
		...
		newPT.setQName(copyQName(origPT.getQName()));
	...
	public static CorrelationSet copyCorrelationSet(CorrelationSet origCorrelationSet) {
		...
		for (Property origProperty : origProperties) {
			newCorrelationSet.getProperties().add(copyProperty(origProperty));
		}
	...
	public static Property copyProperty(Property property) {
		...
		if (property.getQName() != null)
			newProperty.setQName(copyQName(property.getQName()));

		newProperty.setRequired(property.getRequired());

		if (property.getElementType() != null)
			newProperty.setElementType(copyQName(property.getElementType()));
	...
	public static Variable copyVariable(Variable variable) {
		...
		if (origMsg != null) {
			if (origMsg instanceof MessageProxy) {
				...
				newVariable.setMessageType(copyMessage(origMsg));
			}

		} else {
			logger.warn("Variable without messageType is used! Variable should be always of messageType");
		}
		...
		if (variable.getFrom() != null) {
			...
			logger.warn("from in variable: " + variable.getName() + " is not copied.");
		}
